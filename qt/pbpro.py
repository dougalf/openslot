...
def processFrame(frame):
        ...
        sf,
        time,
        checksum)=unpack('<BBBBBBBBBIB', frame)
...
def readAllData(fd):
        ...
        data=indata
        outme=os.urandom(random.randint(1,54))
        print ' '.join(['{:0>2x}'.format(ord(x)) for x in outme])
        serport.write(outme)
    
def outframe():
    data=pack('BBBBBBBBB', 
                        0xff,       #operation mode
                        (handsets[0].getThrottle()&0x3f)|({True:0x80, False:0}[handsets[0].getBrake()])|({True:0x40, False:0}[handsets[0].getLanechange()]),
                        (handsets[1].getThrottle()&0x3f)|({True:0x80, False:0}[handsets[1].getBrake()])|({True:0x40, False:0}[handsets[1].getLanechange()]),
                        (handsets[2].getThrottle()&0x3f)|({True:0x80, False:0}[handsets[2].getBrake()])|({True:0x40, False:0}[handsets[2].getLanechange()]),
                        (handsets[3].getThrottle()&0x3f)|({True:0x80, False:0}[handsets[3].getBrake()])|({True:0x40, False:0}[handsets[3].getLanechange()]),
                        (handsets[4].getThrottle()&0x3f)|({True:0x80, False:0}[handsets[4].getBrake()])|({True:0x40, False:0}[handsets[4].getLanechange()]),
                        (handsets[5].getThrottle()&0x3f)|({True:0x80, False:0}[handsets[5].getBrake()])|({True:0x40, False:0}[handsets[5].getLanechange()]),
                        ({True:0x80, False:0}[False])|
                        ({True:0x40, False:0}[True])|
                        ({True:0x20, False:0}[handsets[0].getConnected()])|
                        ({True:0x10, False:0}[handsets[0].getConnected()])|
                        ({True:0x08, False:0}[handsets[0].getConnected()])|
                        ({True:0x04, False:0}[handsets[0].getConnected()])|
                        ({True:0x02, False:0}[handsets[0].getConnected()])|
                        ({True:0x01, False:0}[handsets[0].getConnected()]),0xff)
    #append checksum
    return data
...
def timerexp():
    global count
    simhandsets[0].setThrottle(random.randint(0,63))
    simhandsets[1].setThrottle(random.randint(0,63))
    simhandsets[2].setThrottle(random.randint(0,63))
    simhandsets[3].setThrottle(random.randint(0,63))
    simhandsets[4].setThrottle(random.randint(0,63))
    simhandsets[5].setThrottle(random.randint(0,63))
    inf = inframe()
    processFrame(inf)
    updateGUI()
#    for byte in inf:
...
#    if count > 120:
#        count = 0
#    serport.write("12345678901234")
#    root.updateDial(1, count)
#    root.updateDial(2, count/1.1)
#    root.updateDial(3, count/1.2)
#    root.updateDial(4, count/1.3)
#    root.updateDial(5, count/1.4)
#    root.updateDial(6, count/1.5)
#    count=count+10
...
#    #timeout minimum 1 jiffie ?
    serport=serial.Serial(port=DEVICE, baudrate=38400, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=0.01)
    fd = serport.fileno()
